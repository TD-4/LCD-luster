...
def main():
    ...
    with torch.no_grad():
        ...
        for img_p in all_images:
            ...
            print("\ntop1:{}  class:{} ;".format(pred_topk[0][0].item(), pred_topk[1][0].item()))
            # CAM图
            ...
            if pred_topk[1][0].item() != 18:  # top1置信度<0.3 pred_topk[0][0].item() < 0.5 and
                print("move other ...")
                # 1、拷贝原图
                shutil.move(os.path.join(input_p), os.path.join(args.outputs, img_p.split("\\")[-1]))
                # 2. 拷贝直方图
                ...
                cv2.imencode('.jpg', img_gray)[1].tofile(
                    os.path.join(args.outputs, input_p.split('\\')[-1][:-4] + ".jpg"))
                # 3、拷贝CAM图
                cv2.imencode('.png', np.array(result)[:, :, ::-1])[1].tofile(
                    os.path.join(args.outputs, input_p.split('\\')[-1][:-4] + ".png"))
...
def parse_arguments():
    ...
    parser.add_argument('-m', '--model', default='pretrained/best_model.pth', type=str, help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-i', '--images', default=r"F:\Data\Screen\20210820\hHYQ", type=str, help='Path to the images to be segmented')
    parser.add_argument('-o', '--outputs', default=r'F:\BD', type=str, help='Output Path')
