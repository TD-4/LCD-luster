...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None):
        ...
        self.lr_scheduler = getattr(utils.lr_scheduler, config['lr_scheduler']['type'])(
            self.optimizer, config["lr_scheduler"]['args']['step_size'], config['lr_scheduler']['args']['gamma'])
        ...
        config_save_path = os.path.join(self.checkpoint_dir, 'configs.json')
